...
class HarmonizationTriplet(L.LightningModule):
    ...
    def __init__(self, base_embedding):
        ...
        self.base_dim = (
            base_embedding.get_sentence_embedding_dimension()
        )  # take base_size

        # hidden layers
        self.fc1 = nn.Linear(
            2
            * self.base_dim,  # multiply by 2 since we use it twice (name, description)
            self.hidden_dim,
        )
    ...
    def forward(self, anchor_name, anchor_desc, pos_name, pos_desc, neg_name, neg_desc):
    ...
    def forward_once(self, name, desc):
        ...
        desc_embedded = torch.from_numpy(self.base_embedding.encode(desc))
        name_embedded = torch.from_numpy(self.base_embedding.encode(name))
...
def train(split_paths, log_path, model_path) -> None:
    """Train a Triplet Neural Network using the previously generated
    synonyms data set.

    Parameters:
    ----------
    split_paths : dict[str, str]
        Dictionary with keys for 'train', 'val', and 'test'. The value for each
        key is a path to a CSV file that contains the data for that split.
    log_path : str
        Path to the directory where training logs will be saved.
    model_path : str
        Path to the .ckpt file where the trained model will be saved.
    """
    # get datasets
    train_dataset = HarmonizationDataset(csv_path=split_paths["train"])
    valid_dataset = HarmonizationDataset(csv_path=split_paths["val"])
    test_dataset = HarmonizationDataset(csv_path=split_paths["test"])
...
def main():
    ...
    for data_type in ["train", "val", "test"]:
        split_paths[data_type] = os.path.abspath(
            os.path.join(
    ...
    log_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "logs", "modeling")
    )
    ...
    train(split_paths, log_path, model_path)
