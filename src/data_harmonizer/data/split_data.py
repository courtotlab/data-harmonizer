...
def create_triplet_template(schema_df: pd.DataFrame) -> pd.DataFrame:
    ...
    unique_field_name_perms = list(itertools.permutations(schema_df["field_name"], 2))

    pos = []
    neg = []
    # using the permutations, add rows to the pos and neg lists
    for unique_field_name_perm in unique_field_name_perms:
        pos.append(schema_df[schema_df["field_name"] == unique_field_name_perm[0]])
        neg.append(schema_df[schema_df["field_name"] == unique_field_name_perm[1]])
...
def create_triplet_df(
    ...
    def anc_template_join(row: pd.Series) -> pd.DataFrame:
        ...
        result = pd.merge(anc_df, triplet_template, how="inner", on="pos_field_name")
        return result
...
def run_split(
    schema_df: pd.DataFrame, synthetic_df: pd.DataFrame
) -> dict[str, pd.DataFrame]:
    """Run the split of synthetic data into triplet datasets

    Parameters
    ----------
    schema_df : pd.DataFrame
        Dataframe containing schema information (e.g. field name, field description).
    synthetic_df : pd.DataFrame
        Dataframe containing synthetic data in the feature columns (e.g. field_name,
        field_description)
    Returns
    ...
    # where n represents the number of columns used as features
    # and 2 represents a positive and negative examples
    schema_df = schema_df[["field_name", "field_description"]]
    ...
    for data_type in ["train", "val", "test"]:
        ...
        triplet_dfs[data_type] = triplet_data_type_df
...
def main():
    ...
    for data_type in data_types:
        save_paths[data_type] = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__),
                "..",
                "..",
    ...
    for data_type in data_types:
        triplet_dfs[data_type].to_csv(save_paths[data_type], index=False)
