...
import pytest
import pandas as pd
from data_harmonizer.data.split_data import (
...
@pytest.mark.parametrize(
    "synthetic_df_1, triplet_template_1", param_create_triplet_template
)
def test_create_triplet_template(synthetic_df_1, triplet_template_1):
    """Test data_harmonizer.data.split_data.create_triplet_template"""

    actual = create_triplet_template(synthetic_df_1)
...
@pytest.mark.parametrize("synthetic_df_2, dataset_dict_2", param_split_data)
def test_split_data(synthetic_df_2, dataset_dict_2):
    ...
    for data_type in ["train", "val", "test"]:
        ...
        # sort dataframe in case values are present but ordered differently
        actual = actual.sort_values(by=actual.columns.tolist()).reset_index(drop=True)
        expected = expected.sort_values(by=expected.columns.tolist()).reset_index(
            drop=True
        )
        ...
        pd.DataFrame(
            {
                ...
                "pos_field_description": ["field_description_5", "field_description_6"],
                "neg_field_name": ["field_name_7", "field_name_8"],
                "neg_field_description": ["field_description_7", "field_description_8"],
            }
        ),
        pd.DataFrame(
            {
...
@pytest.mark.parametrize(
    "synthetic_df_3, triplet_template_3, triplet_df_3", param_create_triplet_df
)
def test_create_triplet_df(synthetic_df_3, triplet_template_3, triplet_df_3):
